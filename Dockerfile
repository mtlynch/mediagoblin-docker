...
# Enable fail2ban to protect against brute forcing HTTP Basic Auth in nginx.
ADD fail2ban.conf /etc/fail2ban/jail.local
ADD fail2ban-filter.conf /etc/fail2ban/filter.d/nginx-auth.conf
RUN apt-get install --yes fail2ban && \
    mkdir --parents /var/run/fail2ban
...
CMD set -xe && \
    sudo htpasswd -c -b -C 31 /etc/nginx/.htpasswd "$BASIC_AUTH_USERNAME" "$BASIC_AUTH_PASS" && \
